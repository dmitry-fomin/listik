...
`Listik` (`listik`) — единственный трекер задач по всем проектам.
...
Связи: `listik dep add <id> <блокер>` (задача ждёт блокер), `listik dep rm <id> <блокер>`.
...
- `listik` в MCP: `claude mcp add listik -- listik mcp`
