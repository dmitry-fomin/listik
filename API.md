...
их ставит доска (вместе с маршрутом), CLI при создании задачи их не выставляет. Панель задачи
даёт выбрать маршрут, пока `route_editable`, и первым пунктом списка — «без маршрута»
(пустая строка в `route`): вместе с маршрутом доска снимает и его метки
`harness:`/`process:`, оставляя остальные метки задачи.
