...
для автостарта. Источник — `routes.json` в корне Listik: у прямых маршрутов (dsh, grok,
codex) в образце уже есть `command`, у конвейеров его нет. При старте сервер один раз
копирует файл в `~/.config/listik/routes.json` (или в `$LISTIK_ROUTES`, если переменная
задана). Уже существующая копия не перезаписывается: argv конвейеров автор вписывает
в неё. Сервер читает файл только при старте (`routes.init_at_startup`), поэтому
