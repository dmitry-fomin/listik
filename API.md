...
| POST | `/api/tasks/{id}/stage` | `holder`, `note`, `harness`, `actor` | следующий этап конвейера s1→s2→s3→s4→done, считает длительность прошлого этапа. `--to <тот же этап>` держателя не снимает: это повторная выдача, а не передача |
