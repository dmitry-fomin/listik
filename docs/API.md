...
нет), `{route}`, `{cwd}`, `{worktree}`, `{branch}`; любая другая фигурная скобка —
...
**Контракт с роем** — то, что каркас обязан делать с меткой `frozen-by:`, а не то, что он
уже делает. На момент этого шага каркас `bin/listik-swarm` метку `frozen-by:` **не
учитывает** и `listik watch` **не зовёт**: замороженная задача (`launched_by` пуст,
держатель снят, карточка открыта) для него — обычный кандидат, и он перезапустит её поверх
грязного дерева. Фильтр по метке и вызов `watch` из цикла добавляет swarm-6 (`listik-dzf0`,
порция a). До этого `listik watch` при живом рое не запускать — только вручную, при
остановленном `listik-swarm`. Требования к каркасу, когда он этот контракт возьмёт:
задачу с меткой `frozen-by:*` не запускать и не считать брошенной; размораживает барьер
(`rebase`, продолжение с диффом, снятие метки); кого размораживать после слияния владельца
`<id>` — задачи проекта с меткой `frozen-by:<id>`.
