...
для автостарта. Источник при работе — таблица `routes` в базе; у каждой записи есть
`command` (конвейер — `claude -p` со скилом `/feature-pipeline:{route}`, прямой —
dsh/grok/codex). `routes.json` в корне Listik или рабочая копия
`~/.config/listik/routes.json` (`$LISTIK_ROUTES`) нужны только для первичного ввоза
при `listik init`/старте сервера и для явного ввоза/вывоза. Непустая таблица при
старте не перезаписывается. Правка записи в базе видна сразу, перезапуск сервера
не нужен; CLI и MCP по stdio читают ту же таблицу при работе с маршрутом.
