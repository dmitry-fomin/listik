...
| `launch_driver` | str? | снимок способа исполнения маршрута: `skill` \| `swarm`; пишет лаунчер в первый захват, `null` — запуска не было (см. «Способ исполнения и рой») |
...
| `portions_stuck` | bool | нарезка зависла: есть незакрытый ребёнок `parent-child`, и ни у одного незакрытого нет признака движения — маршрута вместе с этапом, держателя, `launched_by`; принять порции — `listik portions adopt` (см. «Карточка-порция») |
...
| `store.update_task` (PATCH `/api/tasks/{id}`, `listik set`, `listik_update` в MCP) | `launch_route` — у незакрытой задачи без держателя и живого запуска, ключ должен быть в `routes`; вместе со сменой (или снятием) маршрута снимает `launch_error` и — если флаг поднял отказ автостарта — `needs_owner` (см. «Смена маршрута» ниже) |
...
(доска) или CLI (`listik routes` — только чтение; `listik routes --reimport` —
привести таблицу к файлу поставки, не трогая свои маршруты роя, см.
«CLI: `listik routes`»). Правка
...
У конвейера есть `driver`: `skill` (нет поля — то же самое) или `swarm`; у маршрута роя
он всегда `swarm`.
...
Ответ — последнюю непустую строку stdout — читает сервер, не рой: файл рядом с
`launch_log` с суффиксом `.out` вместо `.log`, последние 64 КиБ. stderr остаётся в `launch_log`. По строке `готово` сервер
...
Запись: `key` (`^[a-z0-9][a-z0-9-]*$`, уникален), `kind` (`pipeline` или
`swarm`; прежний вид `direct` и поле `harness` файл не принимает), `title`, `hint`, `visible` (именно JSON `true`/`false`); у `pipeline`
обязателен `roles` (`spec`/`critic`/`impl`/`judge`), у `swarm` — тоже (формат ячейки
другой — см. «Способ исполнения и рой»); необязательные `icon`,
`command` — непустой массив непустых строк, argv процесса — и `driver`
(`skill`/`swarm`, по умолчанию `skill`; у `kind=swarm` всегда `swarm`). Старое поле `strip` при ввозе игнорируется.

`icon` — уровень маршрута для иконки на доске: `xhigh`, `high`, `medium`, `low`, `xlow` или
`direct` (это уровень, а не вид маршрута); неизвестное значение при ввозе файл не отменяет: запись получает уровень
...
режима скила не делает claim за агента (у роя claim за харнесс роли делает Listik — см.
«Способ исполнения и рой»). После перезапуска сервера `launcher.recover`
...
(`pipeline`/`swarm`), `driver` — способ исполнения (`skill`/`swarm`).
...
последним аргументом) и разбирает **последнюю непустую строку stdout** процесса: stdout идёт в
`*.out` рядом с `launch_log` (`stderr` — в сам `launch_log`). Харнесс `claim`,
`release` и `stage` не вызывает. Ответ — в конце, как вердикт: харнессы вроде
`devin -p` склеивают промежуточные реплики в начале stdout (listik-utw9);
разбор читает последние 64 КиБ.
...
строка ответа; блоки — через пустую строку:

| роль | файл | разделы по порядку |
|---|---|---|
| `spec` | `pipeline-spec-writer.md` | «Вопрос автору — это остановка, а не абзац в отчёте», «Что тебе запрещено», «Файлы» |
| `critic` | `pipeline-critic.md` | «Что ты смотришь», «Что тебе запрещено» |
| `impl` | `pipeline-implementer.md` | «Порядок работы», «Что тебе запрещено», «Если не получается» |
| `judge` | `pipeline-judge.md` | «Что тебе запрещено», «Работа», «Коммит при зелёном вердикте» |

Свой `prompt` ячейки уходит как есть (с подстановками), без критериев. Нет файла агента или
раздела — процесс не поднимается, до `claim`: вопрос человеку
`рой: нет критериев роли <role> (этап <stage>): <нет файла … | в <файл> нет раздела «…»>`.
...
`blocks` — порядок нарезки становится правилом. Родитель с порциями — эпик: его
тип и статус выводятся из порций, в том числе закрытие (см. «Эпик»); вердикт ему
не пишется. Отмена порции из нарезки её не убирает.
...
`start`/`refuse` режима скила карточку роя не
подхватывают.

Ответ «готово» от `impl` на `s3-impl`, когда после него в маршруте нет роли, закрывает
карточку: `stage=done`, `status=done`, держатель снят, вердикта нет, в журнал — строка
`рой: ответ «готово», ролей после s3-impl нет — карточка закрыта`. Коммит в этом случае
делает сама роль `impl`: лаунчер добавляет ей в промпт блок «приёмки после тебя нет —
закоммить свою правку» (кроме ячейки со своим `prompt` — та уходит как есть). «Готово» от
`spec`/`critic`, когда следующей роли нет, — прежний вопрос человеку. Карточка без
`write_scope` на `s3-impl` по-прежнему останавливается вопросом `unscoped`. Так работает
одиночное исполнение: маршрут роя с одной ролью `impl` (или `spec`+`impl` одним харнессом).

Прежний вид `direct` удалён (listik-ar8v). При `listik init`/старте сервера его записи
переводятся на месте тем же ключом в маршрут роя: `kind=swarm`, `driver=swarm`,
`command=null`, `roles.impl = {harness, argv}`, где `argv` — прежняя команда без последнего
элемента-промпта (команда из одного элемента или пустая — только `harness`); промпт прежней
записи не переносится. Харнесс не из каталога заводится в каталоге (`builtin=false`, `argv` —
та же команда без промпта). Задачи и их `launch_route` не трогаются. Колонка `routes.harness`
удалена (`SCHEMA_VERSION` 12, alembic `0010_drop_direct_routes`). Бэкапы `routes.bak-*.json`,
снятые до удаления прежнего вида `direct`, `--reimport --from` не примет.
...
### Смена маршрута («типа запуска») у незакрытой задачи
...
совсем. Сервер разрешает смену у незакрытой задачи (статус не `done`/`cancelled`) без
держателя и без живого запуска (`launched_by` задан, а `launch_finished_at` пуст). Этап и
статусы `in_progress`/`blocked`/`review` не мешают: этап сохраняется, лаунчер подхватит
новый маршрут. Иначе — `400` с кодом `conflict` и текстом
`маршрут нельзя менять: <что именно>. …`; не строка в поле — тоже `400`.

Новый непустой ключ должен быть в таблице `routes`: неизвестный — `400` с кодом
`bad_argument` и текстом `маршрута <ключ> нет в базе; есть: <ключи через запятую>`
(CLI с `--json` — `{"error": {"code": "bad_argument", …}}` и ненулевой код возврата).
Ключ проверяется раньше состояния карточки: у закрытой или взятой задачи неизвестный
ключ даёт именно `bad_argument`, а не «маршрут нельзя менять».

Фактическая смена (новое значение отличается от текущего, в том числе снятие) сбрасывает
`launch_driver` в `null` — это снимок режима старого маршрута, лаунчер снимет его заново.
Этап, держатель, `generation` и остальные поля запуска не трогаются. Тот же ключ
(`route=<текущий>`) или повторное снятие пустого — no-op (`unchanged`): `launch_driver`
не меняется, событий нет. Успешная смена
...
условным `UPDATE … WHERE status NOT IN ('done','cancelled') AND holder пуст AND NOT
(launched_by задан AND launch_finished_at пуст)` (список статусов собирается из
`FINAL_STATUSES`).
...
причине один запрос не может одновременно закрыть задачу или назначить держателя и сменить
маршрут: `route` вместе с `status` в `done`/`cancelled` или непустым `holder` — тоже `400`.
Снять этап (`stage=""`) вместе со сменой маршрута можно.
...
|---|---|---|---|
| PATCH | `/api/routes/{key}` | любые из `title, hint, icon, visible, roles, driver` | `200` с обновлённой записью. `kind, key, command, position` в теле — `400 bad_argument` с именем поля; `roles` пустой (`{}`), `null`, не-объект или негодная ячейка — `400 bad_argument` с путём до поля (`roles.impl.params.a`), запись при этом не меняется; у `driver=swarm` ячейка роли — `{harness, argv?, prompt?}` (харнесс обязан быть в каталоге `harnesses`, `argv`/`prompt` с теми же подстановками, `null`-ячейка — пропуск роли, хотя бы одна роль обязана остаться); `roles.<роль>.skill`, которого нет среди скилов-запускаторов этой установки, — `400` со списком доступных (каталога нет вовсе — проверка не делается); `driver` у `kind=swarm` принимается только `swarm`; постороннее поле — `400` с его именем; пустое тело — `400` «нечего менять»; нет ключа — `404` |
...
ячейки ролей `driver=swarm`; слой базы проверяет
...
| Метод | Путь | Тело | Ответ и правила |
|---|---|---|---|
| GET | `/api/harnesses` | — | `200 {"harnesses": […], "swarm_prompt": "…"}` — записи в порядке `position, key`, у каждой `used_by[]` — где задействован (только `{route, kind:"swarm", role}`); `swarm_prompt` — протокол роли роя, к которому лаунчер при пустом `prompt` ячейки роли дописывает критерии агента роли (см. «Способ исполнения и рой») |
...
переключает принудительно). Вывоз файла из CLI убран: `routes.json` — файл
поставки, его читает первичный ввоз при установке, а дальше маршруты правятся
через HTTP API.

`listik routes --reimport [--from <файл>]` (`POST /api/routes/reimport`, тело `{path?}`; при
`--local` или без сервера — прямо по базе) — привести таблицу к `routes.json` установленной
копии или к файлу `--from`/`path` (любой файл в формате `routes.json`, например бэкап ниже;
CLI приводит путь к абсолютному, сервер читает его сам). `--from` без `--reimport` — ошибка
аргумента (код 2); `path` не строкой — `400 bad_argument`.

Правило для ключей таблицы:
- ключи из файла перезаписываются записями файла (правки на доске для них пропадают) и
  получают позиции `0…n-1` в порядке файла;
- из ключей, которых в файле нет, удаляются только пайплайны-скилы (`kind: pipeline`,
  `driver: skill`); все остальные (`kind: swarm`, `pipeline` с `driver: swarm`, любой
  будущий вид) сохраняются без изменений, кроме `position`: они встают после записей файла
  (`n, n+1, …`) в прежнем взаимном порядке.

До записи таблица снимается в `<каталог данных>/routes.bak-<UTC ГГГГММДДTЧЧММССZ>.json` в
формате `routes.json` (без `position`); занятое имя получает суффикс `-2`, `-3`…; бэкапы не
удаляются. Удаление и запись — одна транзакция: ошибка откатывает таблицу, бэкап остаётся.
В файле у `pipeline` с `driver: swarm`, как у `kind: swarm`, ячейки ролей —
`{harness, argv?, prompt?}`; каталога харнессов у проверки файла нет, поэтому ячейка без
своего `argv` в файле допустима, а харнесс и команду сверяет слой базы при ввозе.

Отчёт: `imported`, `skipped: false`, `source`, `replaced: true`, `kept` — ключи сохранённых
записей, `removed` — ключи удалённых пайплайнов, `backup` — путь бэкапа, и `orphans` —
`{ключ: число задач}` для задач, чей `launch_route` указывает на ключ, которого после
перезаписи нет; сами задачи не меняются, CLI печатает их предупреждением. Человеку CLI
печатает также `сохранены: …`/`удалены: …` (если непусты) и всегда `копия таблицы: <путь>`.
Битый файл — `400 bad_argument`, таблица и бэкапы не тронуты. Установщик спрашивает об этом
при обновлении (`install.sh --routes-reimport yes|no|ask`).
...
- **Зависшая нарезка роя.** Исполнитель `s1-spec` завёл порции, но исход этапа не засчитан
  «готово» — порции без маршрута и этапа, их никто не берёт. Это видно полем
  `portions_stuck`. `listik portions adopt <id>` (`POST /api/tasks/{id}/portions/adopt`)
  принимает нарезку так же, как засчитанное «готово»: держатель родителя снимается, каждой не
  начатой порции (`open`, без этапа, держателя и запуска) — маршрут родителя, первый этап после
  `s1-spec` с ролью и `launch_driver=swarm`; вопрос родителя снимается. Если после `s1-spec`
  роли нет, порции этапа не получают, а на родителе поднимается вопрос. Отказ 409 `conflict`:
  родитель закрыт, не роя, не на `s1-spec`/без этапа, его маршрута нет в базе или не начатых
  порций нет.
- **Одна порция — без дочерней карточки** (listik-zr05). Если у шага ровно одна порция,
  дочерняя карточка не заводится: весь путь `s1→s4` проходит сам шаг, его `spec_path`
  переписывается на файл порции (`stage_launch.merge_single_portion`). Слияние: `spec_path` —
  файл порции, `checklist_path` — её чек-лист, если файл есть; `review_path` не заводится; журнал
  шага `ТЗ шага: <прежний spec_path>` (если он был и другой) и `одна порция <буква> — ведёт сам
  родитель`; пустой `write_scope` шага получает область ребёнка. Уже заведённый ребёнок
  переводится с `parent-child` на `discovered-from` (тип шага возвращается правилом эпика) и,
  если не закрыт, отменяется с `close_reason` `слита в родителя`. Повтор ничего не пишет. Три
  входа: `portions sync` при ровно одном файле `<id>.<буква>.md` (ответ — прежние ключи, где
  `created`/`updated`/`unchanged`/`linked` пусты, `portions` — одна запись с `id` шага, и
  `merged: true`; при двух файлах и больше — прежнее поведение и `merged: false`); засчитанное
  «готово» `s1-spec` роя, когда незакрытая порция одна, не начата и у неё есть `spec_path` —
  после слияния шаг идёт на следующий этап с ролью, как без нарезки; `portions adopt` в том же
  случае — шаг уходит на следующий после `s1-spec` этап с ролью (держатель и вопрос снимаются;
  роли нет — вопрос, `stage: null`), ответ `adopted: []`, `stage` — этап шага, `merged: true`;
  в остальных случаях `adopt` отвечает `merged: false`.
- **`write_scope` порций.** Нарезка роя (засчитанное «готово» `s1-spec` и `portions adopt`)
  отдаёт не начатой порции с пустым `write_scope` область родителя и пишет на порции журнал
  `write_scope унаследован от <родитель>`; свою область порции не перетирает.
...
## Эпик: карточка с подзадачами

Карточка, у которой есть хотя бы один ребёнок по связи `parent-child` (или старой
`parent`), — **эпик**: своей работы у неё нет, её ведут подзадачи. Тип и статус эпика
выводятся из детей (`store.sync_epic`); все такие записи — от `agent:listik`. Пересчёт
идёт, когда связь `parent-child` пишется или снимается (`dep add`/`dep rm`, `create
--parent`), когда удаляют ребёнка и при любой смене статуса ребёнка; вложенные эпики
пересчитываются вверх по цепочке. Импорт (`import-from-bd`) пишет `parent-child` без
пересчёта (`add_dep(..., sync=False)`): он восстанавливает историю как есть. Связь к
закрытой карточке не запрещена. Висячие строки
`deps` без задачи детьми не считаются.

**Тип.** Появились дети, тип не `epic` → тип `epic`, событие `type_change` (`from_value` —
старый тип, `to_value` — `epic`, `note` «подзадачи») и журнал «тип: <старый> → epic
(появились подзадачи)». Детей не осталось, тип `epic` → тип возвращается к прежнему, только
если последнее событие `type_change` карточки — та самая автоматическая смена (`note`
«подзадачи», `to_value` `epic`): событие с `note` «подзадач не осталось» и журнал «тип:
epic → <тип> (подзадач не осталось)». Карточка, созданная эпиком, или эпик, чей тип после
автоматической смены меняли руками, остаётся `epic`.

**Статус** (держатель и этап детей не учитываются):

| Дети | Эпик |
|---|---|
| хотя бы один `in_progress`, `blocked` или `review` | `in_progress` |
| иначе хотя бы один `open` | `open` |
| все закрыты, хотя бы один `done` | `done`, `stage=done`, `close_reason` «подзадачи закрыты: <id>, …» (все дети в порядке создания, включая отменённых), журнал с тем же текстом |
| все `cancelled` | `cancelled`, `close_reason` «подзадачи отменены: <id>, …», журнал; вопроса человеку нет |

Статус уже совпадает с выведенным — записи нет. Закрытый эпик, получивший незакрытого
ребёнка, открывается снова: этап `done` снимается (другой этап не трогается),
`close_reason` сбрасывается.

**В работу не берётся.** Эпик с детьми не попадает в `ready` и в рабочее множество
`waves` (значит, ресурсных рёбер с ним нет, прежние снимает `waves --apply`). `claim`
эпику отказывает — 409 `conflict` «задача <id> — эпик: бери подзадачи: <id>, …»
(незакрытые дети, а если таких нет — все); `force` отказ не обходит.

**Событие `type_change`.** Любая смена `issue_type` через правку карточки пишет событие
`type_change` (`from_value`/`to_value`, `note` — переданная заметка); смена на то же
значение события не пишет.
...
| GET | `/api/routes` | — | `ok, error, path, warnings[], routes[]` — записи таблицы `routes` (`command`, `roles`, `position`, посчитанный `icon`; см. «Маршруты запуска»), у `kind=pipeline` ещё `skill_path` и, если скила нет, `skill_missing: true` (в ответе `visible: false`) — см. «Справочник маршрутов и скилов конвейеров»; `warnings` — замечания ввоза и сверки со скилами (неизвестный `icon` записи: фолбэк по ключу и поле `icon_error`; маршрут без скила: строка про скрытый маршрут); ошибка базы — `ok=false` и текст, а не HTTP-ошибка |
...
| PATCH | `/api/routes/{key}` | `title, hint, icon, visible, roles, driver` (любые) | правка маршрута; `kind, key, command, position` в теле — `400` — см. «Правка, заведение, удаление и порядок маршрутов» |
| POST | `/api/routes` | конвейер: `key` (ключ скила), необязательно `roles`; рой: `kind="swarm"`, `key`, `title`, `roles` | завести маршрут `kind=pipeline` под существующий скил или маршрут роя `kind=swarm` — см. там же |
...
| POST | `/api/routes/reimport` | `{path?}` | привести таблицу к `routes.json` установленной копии или к файлу `path`; свои маршруты роя остаются, бэкап — `backup` в отчёте; см. «CLI: `listik routes`» |
...
Запасные модели, основание выбора и признак переключения описаны в `docs/usage.md`, «Модель роя».
