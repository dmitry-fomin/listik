...
привести таблицу к файлу поставки, не трогая свои маршруты роя и прямых харнессов, см.
...
| Метод | Путь | Тело | Ответ и правила |
|---|---|---|---|
| GET | `/api/harnesses` | — | `200 {"harnesses": […], "swarm_prompt": "…"}` — записи в порядке `position, key`, у каждой `used_by[]` — где задействован (`{route, kind:"direct"}` \| `{route, kind:"swarm", role}`); `swarm_prompt` — протокол роли роя, к которому лаунчер при пустом `prompt` ячейки роли дописывает критерии агента роли (см. «Способ исполнения и рой») |
...
- из ключей, которых в файле нет, удаляются только пайплайны-скилы (`kind: pipeline`,
  `driver: skill`); все остальные (`kind: swarm`, `pipeline` с `driver: swarm`, `direct`, любой
...
| POST | `/api/routes/reimport` | `{path?}` | привести таблицу к `routes.json` установленной копии или к файлу `path`; свои маршруты роя и прямых харнессов остаются, бэкап — `backup` в отчёте; см. «CLI: `listik routes`» |
