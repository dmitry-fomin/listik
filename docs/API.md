...
Listik или рабочая копия `~/.config/listik/routes.json` (`$LISTIK_ROUTES`) — это только
формат ввоза/вывоза: `listik init`/старт сервера ввозят файл в пустую таблицу один раз
(непустая при старте не перезаписывается), а дальше запись правится через API/CLI
(`listik routes`) или переввозится целиком (`listik routes import --replace`). Правка
...
`listik routes` / `listik routes list` — таблица маршрутов (порядок, ключ, вид,
заголовок, скрыт, нет скила); как у всех команд — через сервер, если он поднят, иначе
прямо по базе (`--local` переключает принудительно). `listik routes import [ФАЙЛ]
[--replace]` и `listik routes export [ФАЙЛ]` работают с базой напрямую в обоих режимах
(сервера эти операции не касаются отдельными ручками): `import` без `--replace` на
непустой таблице печатает «маршруты уже есть, нужен --replace» и завершается кодом 1;
файл с неизвестной подстановкой в `command` не ввозится вовсе (код возврата ненулевой,
текст называет подстановку), и после успешного ввоза, если сервер поднят, CLI сам
дёргает `POST /api/notify` (`kind=route`), как это делает MCP по stdio для задач, —
чтобы доска перечитала таблицу. `export` пишет `{"version": 1, "routes": [...]}` (без
`position` — порядок задаёт сам массив) в файл или, без пути, в stdout; выгруженный файл
обязан ввозиться обратно без правок.
...
| POST | `/api/routes` | `key` (ключ скила) | завести маршрут `kind=pipeline` под существующий скил — см. там же |
...
кадр. Нужен он тому, кто пишет в ту же sqlite мимо сервера, — MCP по stdio (`listik mcp`) и
`listik routes import` (пишет таблицу `routes` напрямую): `publish` живёт в процессе сервера,
