...
| POST | `/api/tasks/{id}/portions/adopt` | `actor`, `harness` | принять зависшую нарезку роя (см. «Карточка-порция»): карточка родителя плюс `adopted[]` (id порций, получивших этап) и `stage` (этап порций или `null`, если после `s1-spec` роли нет). 409 `conflict`: родитель закрыт, не роя, не на `s1-spec`, маршрута нет в базе, не начатых порций нет; 404 — задачи нет |
