...
перезаписать таблицу из файла поставки целиком). Правка
...
`listik routes --reimport` (`POST /api/routes/reimport`, при `--local` или без сервера —
прямо по базе) — перезаписать таблицу из `routes.json` установленной копии: старые записи
удаляются, файл ввозится заново (правки на доске пропадают). Отчёт: `imported`, `source`,
`replaced: true` и `orphans` — `{ключ: число задач}` для задач, чей `launch_route` указывает
на ключ, которого после перезаписи нет; сами задачи не меняются, CLI печатает их
предупреждением. Битый файл — `400 bad_argument`, таблица не тронута. Установщик
спрашивает об этом при обновлении (`install.sh --routes-reimport yes|no|ask`).
...
| POST | `/api/routes/reimport` | — | перезаписать таблицу из `routes.json` установленной копии — см. «CLI: `listik routes`» |
