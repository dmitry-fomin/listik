...
для автостарта. Источник — `routes.json` в корне Listik: у каждой записи есть `command`
(конвейер — `claude -p` со скилом `/feature-pipeline:{route}`, прямой — dsh/grok/codex).
При старте сервер один раз копирует файл в `~/.config/listik/routes.json` (или в
`$LISTIK_ROUTES`, если переменная задана). Уже существующая копия не перезаписывается.
Сервер читает файл только при старте (`routes.init_at_startup`), поэтому
