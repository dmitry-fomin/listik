...
Команды `ready`, `list`, `board`, `stats`, `search`, `blocked` и `new`, если проект не задан
явно, определяют его по текущему каталогу. Проект подставляется как значение `--project`,
поэтому вызов из каталога проекта показывает очередь этого проекта, а не всех сразу.
