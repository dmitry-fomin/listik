...
Флаги (флаг важнее переменной окружения):
  ...
  --routes-reimport yes|no|ask
                        ...
                        (LISTIK_ROUTES_REIMPORT), по умолчанию ask; ВНИМАНИЕ: таблица
                        маршрутов в базе стирается целиком — правки на доске и
                        маршруты, которых нет в routes.json (рой и добавленные
                        вручную), пропадут безвозвратно; задачи не трогаются; ask — спросить в /dev/tty, без tty — no
...
ask_routes_reimport() {
    ...
    if ui_menu2 "Перезаписать маршруты из routes.json?" \
            "Таблица маршрутов в базе будет стёрта целиком: правки на доске и маршруты, которых нет в routes.json (рой, добавленные вручную), пропадут безвозвратно. Задачи не тронутся." \
            "Перезаписать|listik routes --reimport" \
            "Не трогать|маршруты в базе останутся как есть" 2; then
        [ "$menu_choice" = 1 ]
        return
    fi
    printf 'Перезаписать маршруты из routes.json? Таблица маршрутов будет стёрта целиком: правки на доске и маршруты не из routes.json пропадут безвозвратно, задачи не тронутся [y/N] ' \
        >/dev/tty 2>/dev/null || return 1
    answer=
    read -r answer < /dev/tty 2>/dev/null || return 1
    case $answer in
