...
Флаги (флаг важнее переменной окружения):
  ...
  --routes-reimport yes|no|ask
                        ...
                        пропадают), свои маршруты роя не из файла
...
ask_routes_reimport() {
    ...
    if ui_menu2 "Перезаписать маршруты из routes.json?" \
            "Пайплайны перезапишутся из routes.json (правки их на доске пропадут), свои маршруты роя, которых нет в файле, останутся. Копия таблицы ляжет в каталог данных (routes.bak-*.json), вернуть — listik routes --reimport --from <копия>. Задачи не тронутся." \
            "Перезаписать|listik routes --reimport" \
            "Не трогать|маршруты в базе останутся как есть" 2; then
        [ "$menu_choice" = 1 ]
        return
    fi
    printf 'Перезаписать маршруты из routes.json? Пайплайны перезапишутся из файла, свои маршруты роя останутся, копия таблицы — в каталоге данных (routes.bak-*.json), задачи не тронутся [y/N] ' \
        >/dev/tty 2>/dev/null || return 1
    answer=
    read -r answer < /dev/tty 2>/dev/null || return 1
    case $answer in
