...
маршруты `routes.json`, а получает черновик задачи
...
def draft(text: str, *, projects: list[dict], cfg: dict | None = None,
    ...
    `store.list_projects`); `routes` — записи `routes.json` (по умолчанию
    текущее состояние сервера). Модель выбирает только из этих списков.
    """
