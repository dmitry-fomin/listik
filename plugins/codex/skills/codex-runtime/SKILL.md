...
- Один прогон — одна задача, продолжения диалога нет: следующий вызов начинается с
  чистого листа, нужный контекст повторяй в новой задаче. (У самого codex есть `resume` и
  `fork` для интерактивных сессий — эта обвязка их не использует, headless-прогон каждый
  раз новый.)
...
какой формулировке задачи. Допустимые значения `--effort` не зафиксированы — они зависят
