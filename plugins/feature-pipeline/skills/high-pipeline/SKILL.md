---
name: high-pipeline
description: "Конвейер реализации на пресете high-pipeline — ТЗ пишет Opus 5.5 на high, ТЗ критикует GLM 5.3 Flash по HTTP, код пишет Opus 5.5 на high, принимает и коммитит Grok 4.7 на xhigh в своём агентном харнессе; основной контекст только маршрутизирует, носит вопросы автору и ведёт журнал. Вызывай, когда шаг спеки или фичу надо провести через ТЗ, критику, реализацию и независимую приёмку и нужен расклад по умолчанию — квоты подписки уходит немного, внешних денег около четырёх долларов на задачу. Запускай только по явному имени: маршрутом карточки Listik (launch_route или метка process:<ключ>) или когда человек назвал пресет; по сходству задачи сам не подхватывай."
...
`pipeline-spec-writer` через `SendMessage` с путём к `$STEPS/$BASE.review-<X>.md` и решениями; после `/clear` —
