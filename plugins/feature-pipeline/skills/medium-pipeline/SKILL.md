...
`pipeline-spec-writer-low` через `SendMessage` с путём к `$STEPS/$BASE.review-<X>.md` и решениями; после `/clear` —
новый запуск в режиме правки (пути к порции, чек-листу и `$STEPS/$BASE.review-<X>.md`, решения). Одна критика
