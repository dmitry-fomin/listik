---
name: xhigh-pipeline
description: "Конвейер реализации на пресете xhigh-pipeline — ТЗ пишет Opus 5.5 на xhigh, ТЗ критикует GLM 5.3 Flash по HTTP, код пишет Opus 5.5 на xhigh локальным субагентом, принимает и коммитит Grok 4.7 на xhigh в своём агентном харнессе; основной контекст только маршрутизирует, носит вопросы автору и ведёт журнал. Вызывай, когда шаг спеки или фичу надо провести через ТЗ, критику, реализацию и независимую приёмку на максимальном усилии каждой роли, потому что ошибка дороже прогона. Внешних денег около четырёх долларов на задачу (оценка Artificial Analysis), квоты подписки уходит больше, чем в любом другом пресете. Запускай только по явному имени: маршрутом карточки Listik (launch_route или метка process:<ключ>) или когда человек назвал пресет; по сходству задачи сам не подхватывай."
...
Самый дорогой пресет: внешних ≈ $4 на задачу (оценка Artificial Analysis, почти всё — судья), квоты подписки
больше всех. Берётся, когда ошибка дороже прогона. Каждая роль на xhigh: ТЗ и код — Opus 5.5 (Anthropic, по
подписке), критика — GLM 5.3 Flash (Z AI), приёмка — Grok 4.7 (SpaceXAI); проверяющие — другие семьи, чем автор.
...
чек-листа, кода и диффа порции** наружу: критику уходят ТЗ и чек-лист, судье — дифф. Секреты в ТЗ не пропускает этап 1, в дифф — границы правки порции. Все команды — из корня основного
дерева.
...
| --- | --- | --- | --- |
| 1. ТЗ и чек-листы | субагент `feature-pipeline:pipeline-spec-writer-xhigh` | **`model: opus` в вызове** (frontmatter — Fable, effort xhigh) → Opus xhigh | `готово` или `вопрос` |
...
Два Claude-субагента ведут себя по-разному нарочно. У автора ТЗ `pipeline-spec-writer-xhigh` во frontmatter
`fable` + `xhigh`, и здесь его **перебивают параметром `model: opus` в вызове**: так и получается Opus xhigh —
`model` перебивает frontmatter, усилие остаётся из него. Забудешь `model` — ТЗ напишет Fable. У исполнителя
`pipeline-implementer-xhigh` во frontmatter уже `opus` + `xhigh`, и `model` ему не передаётся. Усилие у обоих меняется
только сменой агента. У судьи модель и усилие — флаги, без них работу сделает не та модель.
...
`second-opinion:ask` с коротким промптом на `glm`; судья — **настоящим коротким прогоном** через `/grok:delegate` без `--background`:
...
правки (`model: opus`, пути к порции, чек-листу и `$STEPS/$BASE.review-<X>.md`, решения). Одна критика на порцию, поправленное ТЗ не критикуется.
...
`## Listik`). Субагент своей сессии — держатель `claude`: сессия выдаёт карточку себе и держит страховочные
`claim`/`heartbeat`, а `claim`/`journal` субагент пишет сам по скилу `listik:listik`. Порцию он не коммитит — коммит
за судьёй. На повторе после красного — **тот же** субагент через `SendMessage` по его agent id, красные пункты судьи
**дословно**; продолжить нельзя (agent id потерян после `/clear`, `SendMessage` отказал) — **новый** субагент с текущим
текстом задачи и теми же пунктами дословно, строкой в журнал (`pipeline-core.md`, «Шаг 0 — конфиг»).
...
- **красный** — коммита нет, порция на этап 3 с пунктами дословно, в пределах бюджета; повтор идёт
  тем же субагентом через `SendMessage` (правило `pipeline-core.md`, «Шаг 0 — конфиг»), а новый субагент — только
...
| --- | --- | --- |
| ТЗ написано Fable, а не Opus | забыт `model: opus` у автора ТЗ | у `pipeline-spec-writer-xhigh` во frontmatter `fable` — `model: opus` в каждом запуске и в режиме правки |
