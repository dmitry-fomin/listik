...
Модель, вариант и агента не задавай: по общему правилу прогон идёт на модели по умолчанию
(`b.ai/glm-5.3-flash`), поэтому флаги `--model`, `--variant`, `--agent` не добавляй ни при
какой формулировке задачи. Исключение — вызов из пресета feature-pipeline: он обязан
передать требуемые пресетом `--model` и `--variant`, потому что расстановка моделей по
ролям — часть пресета (обоснование в `plugins/feature-pipeline/references/ROLES.md`).
Во всех остальных случаях общий запрет действует.
