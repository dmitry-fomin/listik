...
description: Отдать задачу opencode — второму агентному харнессу, который сам ходит по коду в рабочем каталоге. Каналов два: GLM 5.3 Flash по умолчанию и DeepSeek V4.1 Flash, если человек попросил именно его. Уходит фоновой задачей со своим идентификатором — ты работаешь дальше, пока opencode думает. Сессию можно назвать и потом продолжить тем же именем, не теряя историю. Независимых кусков несколько — запускай несколько прогонов разом, лимита на один нет. Используй, когда нужно обойти незнакомую базу или подсистему целиком, собрать карту, найти все вхождения или получить разбор от другой модели с доступом к файлам. По умолчанию opencode работает только на чтение.
when_to_use: Триггер-фразы — «делегируй opencode», «отдай задачу opencode», «пусть opencode разберётся», «спроси у opencode», «запусти opencode в фоне», «продолжи сессию opencode», «запусти несколько opencode». Явная просьба человека = согласие на запуск. Годится и без упоминания opencode, когда надо обойти незнакомую базу целиком, а тащить её в контекст дорого. Не бери незапрошенные правки кода, мелочь и вопросы синтаксиса или API. Если гипотеза уже есть и нужна проверка — это /opencode:opencode-second-opinion. Управление запущенными задачами — /opencode:opencode-jobs.
argument-hint: "[--write] [--sync] [--session <имя>] [что должен сделать opencode]"
allowed-tools: Agent, Bash(${CLAUDE_PLUGIN_ROOT}/scripts/opencode-run.sh *)
---

Запрос человека: $ARGUMENTS

opencode — не «вторая модель, которой задают вопрос», а второй агент с инструментами: он
читает файлы, грепает, ходит по каталогу и подчиняется тем же `CLAUDE.md`/`AGENTS.md`, что
и ты. Ценность делегирования — в том, что он проходит путь исследования самостоятельно, в
своём контексте, не тратя твой.

Каналов у обвязки два, и выбираются они коротким именем в `--model`:

| Канал | Модель | Когда |
| --- | --- | --- |
| `glm` | `b.ai/glm-5.3-flash` | по умолчанию, флаг не нужен |
| `deepseek` | `b.ai/deepseek-v4.1-flash` | только если человек попросил именно DeepSeek или второй канал |
...
Работает он долго — десятки минут на большую подсистему. `opencode run` сам по себе не
...
1. **Запусти.** Вызов Bash возвращает идентификатор задачи одной строкой:
   ...
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/scripts/opencode-run.sh run --background --session "<имя-сессии>" --label "<о чём задача>" [опции] <<'TASK'
   <текст задачи>
   TASK
   ```
...
4. **Поставь ожидание фоновым вызовом Bash** (`run_in_background: true`), чтобы тебя
   ...
   Уведомление о завершении этого вызова и есть сигнал «opencode закончил». Задач
...
6. **Забери ответ:** `${CLAUDE_PLUGIN_ROOT}/scripts/opencode-run.sh result <job-id>`.
   Id сессии opencode после завершения — в `status --json` поле `opencode_session`.
   Продолжение той же сессии:

   ```bash
   ${CLAUDE_PLUGIN_ROOT}/scripts/opencode-run.sh resume --session "<имя-сессии>" --background --label "<продолжение>" <<'TASK'
   <текст продолжения>
   TASK
   ```
   ...
   `opencode:opencode-runner` вызовом `Agent` с `subagent_type: "opencode:opencode-runner"`,
   передав ему идентификатор.
...
- **Держи пачку небольшой** — 2–4 задачи. Их ответы придётся сводить в твоём контексте, а
  каждый ответ opencode — это цельное финальное сообщение, не выжимка.
...
**`opencode:opencode-runner` — субагент, а не скил.** Не вызывай
`Skill(opencode:opencode-runner)` и не вызывай `Skill(opencode:opencode-delegate)` изнутри
этого скила: второе перезапускает сам этот скил и вешает сессию. Скил выполняется в
основном треде, поэтому инструмент `Agent` тебе доступен — форкнутые субагенты его не видят.
...
2. **Границы области.** Каталог или список файлов, за пределы которых выходить не нужно.
   Заодно запрети трогать `.env`, `*.key`, `*.pem`, `credentials.json` — opencode читает
   файлы сам, и это единственное место, где такой запрет можно поставить.
...
| --- | --- | --- |
| (ничего) | только чтение: правка и bash запрещены, остаются read/grep/glob/list | по умолчанию, для любого исследования |
...
**`--bash` — это уже не «только чтение».** Песочницы уровня ОС у opencode нет, и с
разрешённым bash модель может записать файл через обычный `>`. Поэтому `--bash` добавляй
осознанно и говори человеку, что запись в этом режиме технически возможна.
...
Красные линии проекта действуют и внутри opencode: он не должен коммитить, пушить, удалять
...
| «продолжи тот разбор», названо имя прошлой сессии | `resume --session <имя>` вместо `run` — канал сессии наследуется, повторять `--model` не нужно |
| указан каталог или подсистема | `--cwd <путь>` |
| «надолго», «пусть роется сколько нужно» | `--timeout 0` |
| «через deepseek», «спроси DeepSeek», «вторым каналом» | `--model deepseek` |

Канал и уровень усилия сам не выбирай: по умолчанию прогон идёт на `b.ai/glm-5.3-flash`,
`--variant` не добавляй ни при какой формулировке задачи, а `--model` — только в двух
случаях. Первый: человек прямо назвал канал («через deepseek», «спроси DeepSeek») — тогда
`--model deepseek`, и ответ подавай с оговоркой про склонность выдумывать. Второй: вызов
из пресета feature-pipeline — такой вызов обязан передать требуемые пресетом `--model` и
`--variant`, потому что расстановка моделей по ролям — часть пресета (обоснование в
`plugins/feature-pipeline/references/ROLES.md`). Сам по себе, без просьбы и без пресета,
канал не меняй.

## Что делать с ответом

- Ответ opencode показывай дословно, помечая, что это результат другого харнесса, а не твой
...
- Если opencode не справился — доложи это, а не доделывай задачу молча вместо него.
...
Полный контракт скрипта — в скиле `opencode-runtime`; управление списком задач —
`/opencode:opencode-jobs`.
