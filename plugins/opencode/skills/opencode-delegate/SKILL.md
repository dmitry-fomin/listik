...
своём контексте, не тратя твой. Модель по умолчанию — `b.ai/glm-5.3-flash`.
...
| «продолжи тот разбор», названо имя прошлой сессии | `resume --session <имя>` вместо `run` |
| указан каталог или подсистема | `--cwd <путь>` |
| «надолго», «пусть роется сколько нужно» | `--timeout 0` |

Модель и уровень усилия не выбирай: по умолчанию прогон идёт на `b.ai/glm-5.3-flash`,
флаги `--model` и `--variant` не добавляй ни при какой формулировке задачи. Исключение —
вызов из пресета feature-pipeline: такой вызов обязан передать требуемые пресетом
`--model` и `--variant`, потому что расстановка моделей по ролям — часть пресета
