...
5. **Запусти прогон фоном, без `--write` и без `--bash`:**
   `opencode-run.sh run --background --label "<о чём гипотеза>"`. Модель не задавай —
   прогон идёт на `b.ai/glm-5.3-flash`. Идентификатор задачи назови человеку и продолжай
...
`resume --session <имя>`, чтобы не пересказывать контекст заново.
