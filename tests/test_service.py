...
class ServiceCliTestCase(unittest.TestCase):
    ...
    def test_darwin_install_no_load_writes_plist(self) -> None:
        ...
        service_path = data["EnvironmentVariables"]["PATH"].split(os.pathsep)
        self.assertIn(str(self.home / ".local" / "bin"), service_path)
        self.assertIn("/opt/homebrew/bin", service_path)
    ...
    def test_linux_install_writes_unit_and_reloads(self) -> None:
        ...
        self.assertIn(str(self.home / ".local" / "bin"), text)
        self.assertIn("/home/linuxbrew/.linuxbrew/bin", text)
