 ...
 *
 * Заведение — первичное действие раздела «Завести маршрут» и «+» в заголовке
...
/*
 * Карточка роя сохраняется сама, но правку, которую сервер не
 * примет (команда с ошибкой, все роли сняты), на сервер не пускает — она при
...
/** Несохранённая правка есть у карточки с редактируемым составом — не у конвейера. */
const hasUnsaved = computed(() => cardDirty.value && Boolean(selectedSwarm.value))
