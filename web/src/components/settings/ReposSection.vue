...
/**
 * Раздел «Репозитории» страницы настроек (`views/SettingsPage.vue`): доска
 * показывает ровно те проекты, что лежат в таблице `projects` и не скрыты,
 * поэтому добавление/удаление репозитория — операции над проектом, а не фильтр
 * по задачам.
 *
 ...
 *
 * Список читается в `onMounted`: раздел пересоздаётся при каждом заходе (в
 * `SettingsPage.vue` он под `v-if`), значит и список свежий, и локальное
...
const formSlug = ref('')
const formTitle = ref('')
/** Ошибка сервера показывается в самом окне: раздел за оверлеем не виден. */
const formError = ref<string | null>(null)
/** Чем кончилась правка — плашка в разделе, когда окно уже закрыто. */
...
async function toggleArchived(project: ProjectRow, archived: boolean): Promise<void> {
  ...
  await store.setProjectArchived(project.slug, archived)
  action.value = null
}
...
/**
 * Ошибку показывает само окно: раздел «Репозитории» под оверлеем, и его плашка
...
<template>
  ...
  <!-- Добавление и правка репозитория — одно окно (тот же приём, что у
       подтверждений ниже): форма живёт, пока идёт запрос, и несёт свои ошибки —
       за оверлеем плашку раздела не видно. Различаются только набор полей
       (в правке slug уже занят и не меняется) и адрес запроса. -->
  <UiFormModal
    v-model="formOpen"
  ...
  <!-- Подтверждения — соседи формы, а не её содержимое: оба оверлея
...
/* Список прокручивается вместе со страницей: своей прокрутки у него больше нет
   (была нужна, пока раздел жил в модалке). */
