...
const sections: Record<string, { title: string; description: string; actionLabel?: string }> = {
  ...
  routes: {
    title: 'Маршруты',
    description:
      'Маршрут — заготовка запуска задачи. Конвейер приходит из поставки: этапы ведёт claude внутри одного процесса, меняются только название, подпись и иконка. Рой Listik водит сам — отдельный харнесс на каждый этап, состав выбирается здесь.',
    actionLabel: 'Завести маршрут',
  },
  harnesses: {
    title: 'Харнессы',
    description:
      'Харнесс — исполнитель: CLI-процесс, который ведёт роль роя. Конвейеры харнессы не используют — там этапы ведёт claude.',
